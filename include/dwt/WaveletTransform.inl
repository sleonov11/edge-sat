template <typename T>
inline void dwt_1d (std::span<T> src, std::span<T> res) {
    size_t n = src.size();
...
void dwt_2d (Image<T>& img) {
    ...
    std::vector<T> scratch(std::max(w, h));
    std::span<T>scratch_span(scratch);

    for (size_t y = 0; y < h; ++y) {
        auto ry = img.row(y);
        dwt_1d(ry, scratch_span.subspan(0,w));
        std::copy(scratch.begin(), scratch.begin() + w, ry.begin());
    }
    ...
    for (size_t x = 0; x < transposed.height(); ++x) {
        ...
        dwt_1d (span_x, scratch_span.subspan(0,h));
        std::copy (scratch.begin(), scratch.begin() + h, span_x.begin());
    }
